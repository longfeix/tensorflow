...
def create_keras_model(fingerprint_input, model_settings, is_training):
  ...
  """
  model = Sequential()
  model.add(Reshape(-1, input_time_size, input_frequency_size, 1))
  model.add(Conv2D(64, (20, 8), activation='relu', border_mode='same'))
  model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
  model.add(Dropout(0.4))

  model.add(Conv2D(64, (10, 4), activation='relu', border_mode='same'))
  model.add(MaxPooling2D(pool_size=(2, 2), padding='same'))
  model.add(Dropout(0.4))
...
def create_conv_model_orig(fingerprint_input, model_settings, is_training):
...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  # NEW START
  third_filter_width = 8
  third_filter_height = 20
  third_filter_count = 64
  third_weights = tf.Variable(
      tf.truncated_normal(
          [third_filter_height, third_filter_width, first_filter_count, third_filter_count],
          stddev=0.01))
  third_bias = tf.Variable(tf.zeros([third_filter_count]))
  third_conv = tf.nn.conv2d(max_pool, third_weights, [1, 1, 1, 1],
                            'SAME') + third_bias
  ...
  max_pool2 = tf.nn.max_pool(third_dropout, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
  # NEW END
  ...
  second_weights = tf.Variable(
      tf.truncated_normal(
          [
              second_filter_height, second_filter_width, third_filter_count,
              second_filter_count
          ],
          stddev=0.01))
  second_bias = tf.Variable(tf.zeros([second_filter_count]))
  second_conv = tf.nn.conv2d(max_pool2, second_weights, [1, 1, 1, 1],
                             'SAME') + second_bias
  second_relu = tf.nn.relu(second_conv)
  if is_training:
    second_dropout = tf.nn.dropout(second_relu, dropout_prob)
